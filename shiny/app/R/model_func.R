model <- function(t, t0, parms) {
  with(as.list(c(t0, parms)), {
    ...
    dI_on_sym <- (1-p_asympt_stu)*(1/latent*E_on - testing*(1-p_asympt_stu)*sensitivity*I_on - 1/infectious*I_on - screening*I_on*sensitivity)
    ...
    dI_off_sym <- (1-p_asympt_stu)*(1/latent*E_off - testing*(1-p_asympt_stu)*I_off*sensitivity - 1/infectious*I_off - screening*I_off*sensitivity)
    ...
    dI_saf_sym <- (1-p_asympt_saf)*(1/latent*E_saf - testing*(1-p_asympt_saf)*I_saf*sensitivity- 1/infectious*I_saf - screening*I_saf*sensitivity)
    ...
    state_list <- c(dS_on,dE_on,dI_on,dP_on,dR_on,dIcum_on, dPcum_on, dQ_on, dQcum_on, dHcum_on, dDcum_on,
                    ...
                    dTest, dI_on_sym, dI_off_sym, dI_saf_sym)
    out <- list(state_list)
    return(out)
  })
}
