...
We start by simulating transmission on campus in which no diagnostic control measures are in place (no testing, isolation, contact tracing, or quarantine). 
Figure \@ref(fig:npifig) shows total number of infections among students and staff for different assumed strengths of general non-pharmaceutical interventions (NPI), with other parameter values at their base values. Table \@ref(tab:basetable) shows the distribution of outcomes when sampling over distributions for each parameter. 
...
We next explored a wide range of screening intervals, from weekly to once during the semester. Figure \@ref(fig:screeningfig) shows total number of infections among students and staff for different screening intervals, with other parameter values at their base values. Table \@ref(tab:screentable) shows the distribution of outcomes for a 30-day screening interval when sampling over distributions of the parameters. 
...
Next, we consider a testing-only strategy, wich also includes contact tracing and quarantine as measures. Assuming different delays between receiving the test and receiving the results, results are shown Figure \@ref(fig:testingfig). We again plot the total number of infections among students and staff, with other parameter values at their base values. Table \@ref(tab:testtable) shows the distribution of outcomes for a 4-day testing delay when sampling over distributions of the parameters. 
...
The impact of testing is highly sensitive to the success of contact tracing, which can overwhelm the impact of quarantine of the tested and isolation of the cases themselves.
Figure \@ref(fig:tracingfig) shows the relationship between “contact tracing” success and cumulative incidence assuming a 4-day delay in testing/quarantine following symptoms. Although the testing interval can reduce the cumulative incidence, the greater impact of this testing scenario is achieved by the number of contacts reached.


```{r tracingfig,  fig.cap=' Impact of testing, contact tracing and quarantine for different levels of contacts reached.', echo=FALSE}
knitr::include_graphics(here('figures/tracing_fig.png'))
```
