model <- function(t, t0, parms) {
  with(as.list(c(t0, parms)), {
    ...
    # dP_on <- testing*(1-p_asympt_stu)*sensitivity*I_on + screening_on*(E_on+I_on)*sensitivity - 1/isolation*P_on
      dP_on <- testing*(1-p_asympt_stu)*sensitivity*I_on 
    #  + screening*(E_on+I_on)*sensitivity - 1/isolation*P_on
    ...
    # dPcum_on <- testing*(1-p_asympt_stu)*I_on*sensitivity + screening_on*(E_on+I_on)*sensitivity
      ...
      #+ screening_on*(E_on+I_on)*sensitivity
      
    # dQcum_on <- testing*(1-p_asympt_stu)*I_on*sensitivity*contacts*p_contacts_reached + (E_on+I_on)*screening_on*sensitivity*contacts*p_contacts_reached
