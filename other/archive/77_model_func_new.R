covid_model <- function(t, t0, parms) {
  with(as.list(c(t0, parms)), {
    ...
    lam_on = (1-eff_npi)*(beta_student_to_student*(Iasym_off+Isym_off) +beta_on_to_on*(Iasym_on+Isym_on) + beta_saf*(Iasym_saf+Isym_saf))
    ...
    #infectious asymptomatic and symptomatic. veryone is screened, only symptomatic are tested.
    ...
    #one can end up in P by either being symptomatic infected and tested, or in any infection state (E, Iasym, Isym) and screened
    #after isolation ends, individuals move to R
    dP_on <- testing*sensitivity*Isym_on + screening*sensitivity*(E_on+Isym_on+Iasym_on) - 1/isolation*P_on
    
    #contact traced and quarantined individuals. First term are S individuals, 2nd term are E individuals
    #this happens if testing identifies cases, for each such a case, a fraction of contacts is quarantined
    #contact tracing and quarantine is not applied for screening 
    ...
    #force of infection, reduced by effectiveness of NPIs
    lam_off = (1-eff_npi)*(beta_student_to_student*(Iasym_off+Isym_off) + (beta_student_to_student)*(Iasym_on+Isym_on) + beta_saf*(Iasym_saf+Isym_saf) )
    ...
    lam_saf = (1-eff_npi)*(beta_saf*(Iasym_on+Isym_on) + beta_saf*(Iasym_off+Isym_off) + beta_saf*(Iasym_saf+Isym_saf))
