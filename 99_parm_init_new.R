...
setpars_ini <- function(school)
{
  ...
  parameter_table <- read.csv("99_ParameterTable_new.csv")
  ...
  p_asympt_stu <- as.numeric(1-parvals["p_sympt_stu"])                  
  p_asympt_saf <- as.numeric(1-parvals["p_sympt_saf"]) 
  ...
  parvals = c(parvals,p_asympt_stu = p_asympt_stu,
                      p_asympt_stu = p_asympt_saf,
  ...
  I_on = 1; I_off = 1; I_saf = 1;
  
  ## Initial conditions to model
  ini_cond <- c(S_on=as.numeric(parvals["N_on"]-I_on),        # number initially susceptible
                        ...
                        S_off=as.numeric(parvals["N_off"]-I_off),
                       ...
                       S_saf=as.numeric(parvals["N_saf"]-I_saf),
                       E_saf = 0,
                         I_saf = I_saf,
                         P_saf = 0,
                         R_saf = 0,
