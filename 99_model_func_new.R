covid_model <- function(t, t0, parms) {
  with(as.list(c(t0, parms)), {
    ...
    lam_on = (1-eff_npi)*(beta_student_to_student*I_off/N_off + (beta_student_to_student+beta_on_to_on)*I_on/N_on + beta_saf*I_saf/N_saf)
    
    dS_on <- -lam_on*S_on - community*S_on - testing*(1-p_asympt_stu)*I_on*sensitivity*(contacts-Rp_on_to_on-R0_student_to_student)*p_contacts_reached + 1/isolation*Q_on*((contacts-Rp_on_to_on-R0_student_to_student)/contacts)
    
    dE_on <-  lam_on*S_on + community*S_on - 1/latent*E_on  - screening*E_on*sensitivity - testing*(1-p_asympt_stu)*I_on*sensitivity*(Rp_on_to_on+R0_student_to_student)*p_contacts_reached #last term represents contract tracing
    ...
    dR_on <- 1/infectious*I_on + 1/isolation*P_on + 1/isolation*Q_on*((Rp_on_to_on+R0_student_to_student)/contacts)
